...
do
  ...
  ( SCHEMA_CONSTRAINTS=0 DEPENDENT_GROUPBY=1 JOIN_TO_SEMI=1 JOIN_TO_PREDICATE=1 VALIDATION_LOOPS=1 "${build_folder}"/"$benchmark" -s ${sf} -r ${runs} -t ${runtime} --scheduler --clients 1 --cores ${num_phy_cores} -m Ordered -w ${warmup_seconds} -o "${build_folder}/benchmark_plugin_results/${benchmark}_${commit}_no_pruning_mt_ordered_s${sf}_plugin.json" -p "${build_folder}/lib/libhyriseDependencyDiscoveryPlugin.${lib_suffix}" 2>&1 ) | tee "${build_folder}/benchmark_plugin_results/${benchmark}_${commit}_no_pruning_mt_ordered_s${sf}_plugin.log"
  ...
  ( SCHEMA_CONSTRAINTS=0 DEPENDENT_GROUPBY=0 JOIN_TO_SEMI=0 JOIN_TO_PREDICATE=1 VALIDATION_LOOPS=1 "${build_folder}"/"$benchmark" -s ${sf} -r ${runs} -t ${runtime} --scheduler --clients 1 --cores ${num_phy_cores} -m Ordered -w ${warmup_seconds} -o "${build_folder}/benchmark_plugin_results/${benchmark}_${commit}_no_pruning_mt_ordered_s${sf}_plugin_jtp.json" -p "${build_folder}/lib/libhyriseDependencyDiscoveryPlugin.${lib_suffix}" 2>&1 ) | tee "${build_folder}/benchmark_plugin_results/${benchmark}_${commit}_no_pruning_mt_ordered_s${sf}_plugin_jtp.log"
done


echo "Running hyriseBenchmarkJoinOrder for $commit... (multi-threaded, ordered, 1 client) NO SCHEMA CONSTRAINTS, PLUGIN, ALL ON"
( SCHEMA_CONSTRAINTS=0 DEPENDENT_GROUPBY=1 JOIN_TO_SEMI=1 JOIN_TO_PREDICATE=1 VALIDATION_LOOPS=1 "${build_folder}"/hyriseBenchmarkJoinOrder -r ${runs} -t 60 --scheduler --clients 1 --cores ${num_phy_cores} -m Ordered -w ${warmup_seconds} -o "${build_folder}/benchmark_plugin_results/hyriseBenchmarkJoinOrder_${commit}_no_pruning_mt_ordered_plugin.json" -p "${build_folder}/lib/libhyriseDependencyDiscoveryPlugin.${lib_suffix}" 2>&1 ) | tee "${build_folder}/benchmark_plugin_results/hyriseBenchmarkJoinOrder_${commit}_no_pruning_mt_ordered_plugin.log"
...
( SCHEMA_CONSTRAINTS=0 DEPENDENT_GROUPBY=0 JOIN_TO_SEMI=0 JOIN_TO_PREDICATE=1 VALIDATION_LOOPS=1 "${build_folder}"/hyriseBenchmarkJoinOrder -r ${runs} -t 60 --scheduler --clients 1 --cores ${num_phy_cores} -m Ordered -w ${warmup_seconds} -o "${build_folder}/benchmark_plugin_results/hyriseBenchmarkJoinOrder_${commit}_no_pruning_mt_ordered_plugin_jtp.json" -p "${build_folder}/lib/libhyriseDependencyDiscoveryPlugin.${lib_suffix}" 2>&1 ) | tee "${build_folder}/benchmark_plugin_results/hyriseBenchmarkJoinOrder_${commit}_no_pruning_mt_ordered_plugin_jtp.log"
